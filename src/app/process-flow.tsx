...
type ProcessNodeVariant = "pipeline" | "hub" | "sensor" | "agent-up" | "agent-down";
...
function ProcessNode({ data }: NodeProps<Node<ProcessNodeData>>) {
  ...
  return (
    <div className={`process-node ${data.status}${data.selected ? " node-selected" : ""}`}>
      {v === "pipeline" && (
        <>
...
/** Fixed grid so the brew path, telemetry stack, and agents stay aligned. */
const PX = 212;
const Y_MAIN = 96;
