...
import { brewDayWorkflow, sensorReadingSignal } from "../temporal/workflows";
...
async function run() {
  ...
  for (let tick = 0; tick < 4; tick += 1) {
    await fermentation.signal(sensorReadingSignal, createSensorReading({ batchId, scenario: "normal", tick }));
    await sleep(500);
  }
  await fermentation.signal(sensorReadingSignal, createSensorReading({ batchId, scenario: "temp_spike", tick: 4 }));
  await fermentation.signal(sensorReadingSignal, createSensorReading({ batchId, scenario: "temp_spike", tick: 5 }));
